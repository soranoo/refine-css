...
Deno.test("cssUnescape reverses cssEscape", () => {
  const testCases = ["test", "-test", "0test", "-0test", "test.class", "special@chars"];
...
Deno.test("stringifySelectorComponent returns correct string representations", () => {
  assertEquals(stringifySelectorComponent({ type: "class", name: "test" }), ".test");
  assertEquals(stringifySelectorComponent({ type: "id", name: "test" }), "#test");
  assertEquals(stringifySelectorComponent({ type: "type", name: "div" }), "div");
...
Deno.test("generateHash produces consistent hashes", () => {
  ...
  assertEquals(hash1, hash2, "Hash should be consistent for the same input string");
});
...
Deno.test("generateHash uses seed for hash generation", () => {
  ...
  assertEquals(hash1 !== hash2, true, "Different seeds should produce different hashes");
