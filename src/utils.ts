...
export const generateHash = (value: string, seed?: number): string => {
  if (!hashInstance) {
    throw new Error("Hash instance not initialized. Call initializeHash() first.");
  }
  ...
  if (firstCharCode < LOWERCASE_A_CHARCODE || firstCharCode > LOWERCASE_Z_CHARCODE) {
 ...
 * ```
 */
export const stringifySelectorComponent = (selectorComponent: SelectorComponent) => {
  ...
  throw new Error(`Unknown selector type: ${JSON.stringify(selectorComponent)}`);
};
